...
public class DatabaseMigrationTool {
            ...
            try (Statement statement = connection.createStatement()) {
                ...
                for (String instruction : instructions) {
                    if (!instruction.trim().isEmpty()) {
                        System.out.println(" Exécution: " + instruction.trim().substring(0,
                                Math.min(50, instruction.trim().length())) + "...");
                        statement.execute(instruction);
                        count++;
                    }
                }
                System.out.println("✅ Migration terminée avec succès ! " + count + " instructions exécutées.");
            }
    ...
    public static void main(String[] args) {
        // Exemple d'utilisation avec H2 (pour les tests)
...
//        DatabaseMigrationTool migrationTool = new DatabaseMigrationTool(
//                "jdbc:mysql://localhost:3306/reservation-salles",
//                "root",
//                "password"
//        );
//        migrationTool.executeMigration();
